...
import net.mcreator.thenine.block.LemonLogBlock;
import net.mcreator.thenine.block.LemonLeavesBlock;
import net.mcreator.thenine.block.CherryDirtBlock;
...
public class LemonTreeBiomeBiome extends TheNineModElements.ModElement {
	@ObjectHolder("the_nine:lemon_tree_biome")
	public static final CustomBiome biome = null;
	public LemonTreeBiomeBiome(TheNineModElements instance) {
		super(instance, 81);
	}
		...
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							CherryDirtBlock.block.getDefaultState(), CherryDirtBlock.block.getDefaultState())));
			setRegistryName("lemon_tree_biome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(LemonLogBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(LemonLeavesBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS_BLOCK.getDefaultState().getBlock() || ground == CherryDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
									|| ground2 == CherryDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), CherryDirtBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == LemonLogBlock.block.getDefaultState().getBlock()
					...
					|| blockType == CherryDirtBlock.block.getDefaultState().getBlock();
		}
