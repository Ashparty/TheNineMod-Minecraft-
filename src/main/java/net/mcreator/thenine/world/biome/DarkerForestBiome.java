...
import net.mcreator.thenine.block.DarkerLogBlock;
import net.mcreator.thenine.block.DarkerLeavesBlock;
...
public class DarkerForestBiome extends TheNineModElements.ModElement {
	@ObjectHolder("the_nine:darker_forest")
	public static final CustomBiome biome = null;
	public DarkerForestBiome(TheNineModElements instance) {
		super(instance, 360);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(DarkerLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(DarkerLeavesBlock.block.getDefaultState()))).baseHeight(4)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == DarkerLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, DarkerLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DarkerLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DarkerLogBlock.block.getDefaultState().getBlock()
					|| blockType == DarkerLeavesBlock.block.getDefaultState().getBlock()
